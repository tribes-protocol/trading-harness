...
description: >-
  ...
  URL (use zipbox-websearch); deep multi-page reading, crawling, or JS-gated pages (use
  zipbox-browser);
...
`tribes-cli web-search` requires an auth token (run `tribes-cli login` once if it fails with
auth errors);
