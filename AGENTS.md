...
Everything the trading agent can do is a subcommand of one CLI. `src/cli/Tribes.ts` is the single entry point: it composes one `build...Command()` builder per group: Wallet, Hyperliquid, Transaction, SpotTrading, News, Macros, Candles, Token, WebSearch, Prediction. `bootstrap.sh` compiles this into a native `tribes-cli` binary on PATH so the agent runs `tribes-cli <group> <command> ...` with no per-call transpile or `@/` alias resolution. `macros market` fetches FRED directly and `candles` fetches unified OHLCV directly, keyed on the `AssetIdentity` kind (`--kind token|perp|stock` → BirdEye / Hyperliquid / Marketstack; symbol→address resolution, timeframe aggregation, null-strip) — both inline, using the provider keys. The analyst/data skills otherwise call their providers directly rather than routing through the CLI.
