...
```bash
bun .pi/skills/prediction/src/cli/Prediction.ts <action> [options]
```
...
```bash
bun .pi/skills/prediction/src/cli/Prediction.ts search --query "crypto regulation stablecoin bill" --limit-per-type 10
bun .pi/skills/prediction/src/cli/Prediction.ts list-events --active true --closed false --limit 10 --order volume --ascending false
bun .pi/skills/prediction/src/cli/Prediction.ts list-markets --closed false --limit 20 --order volume --ascending false
```
...
```bash
bun .pi/skills/prediction/src/cli/Prediction.ts search --query "Fed rate cut June" --limit-per-type 10
bun .pi/skills/prediction/src/cli/Prediction.ts search --query "CLARITY Act crypto" --limit-per-type 10
```
...
```bash
bun .pi/skills/prediction/src/cli/Prediction.ts list-events \
  --active true \
  --closed false \
  --limit 25 \
  --order volume \
...
```bash
bun .pi/skills/prediction/src/cli/Prediction.ts get-event --event-id 12345
bun .pi/skills/prediction/src/cli/Prediction.ts get-event --event-slug some-polymarket-event-slug
```
...
```bash
bun .pi/skills/prediction/src/cli/Prediction.ts list-markets \
  --closed false \
  --limit 20 \
  --order volume \
  --ascending false
...
```bash
bun .pi/skills/prediction/src/cli/Prediction.ts get-market --market-id 12345
bun .pi/skills/prediction/src/cli/Prediction.ts get-market --market-slug some-polymarket-market-slug
```
...
- Entry: `.pi/skills/prediction/src/cli/Prediction.ts`
- Service: `.pi/skills/prediction/src/services/Prediction.ts` (inline HTTP)
- Types: `.pi/skills/prediction/src/types/Prediction.ts` (Zod schemas + option types)
- Enrichment: `.pi/skills/prediction/src/utils/Prediction.ts` (`leadingMarket` via core `selectLeadingMarket`)
