...
import { DEFAULT_NOTIFY_TITLE } from '@/types/Notify'
...
export class NotifyService {
   ...
   *
   * A message with neither an explicit title nor a subtitle emits the bare
   * OSC 9 form rather than OSC 777. That is what this repo emitted before the
   * backend rewrite, and receivers keyed to it must keep working.
   */
  private async sendOsc(request: NotifyRequest): Promise<void> {
    if (request.title === DEFAULT_NOTIFY_TITLE && !request.subtitle) {
      this.writeToTerminal(buildOscBareNotification(request.message))
      return
    }
    const body = joinBody([request.subtitle, request.message], ' — ')
