...
`commodity-analyst`, `macros`, `news`, `prediction`, `wallet-analyst`, `zipbox-x`,
`zipbox-websearch`,
...
a control-plane billing entry) is escalated to the human via the Head of Desk with the `zipbox-notify`
skill.
...
3. **Portfolio discrepancies** (venue state ≠ expected state): Position Monitor → Portfolio
   Manager → halt new instructions for the affected asset → notify the human (`zipbox-notify` skill)
...
| `intel-social-sentiment`    | `zipbox-x` reads, `zipbox-websearch`                                                                                            | X proxy, Tavily                                             |
...
| `eng-provider-dd`           | (documentation-first provider DD; committed records under docs/org/providers/) `zipbox-websearch`                               | Tavily                                                      |
...
| Historical sentiment / odds time series                                                                 | News API is point-in-time; Polymarket CLOB history unwrapped                                                                                      | Departments snapshot per cycle under `.tribes/org/snapshots/` (`--out` where supported; stdout redirect for prediction and `zipbox-websearch`, neither of which has an `--out` — `zipbox-websearch` is a plain `curl` whose JSON goes to stdout, exactly like the `tribes-cli web-search` it replaces, so the redirect is unchanged) |
